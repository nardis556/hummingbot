...
class IdexAPIOrderBookDataSourceUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.matic_order_book_data_source: IdexAPIOrderBookDataSource = IdexAPIOrderBookDataSource(
            cls.matic_sample_pairs, domain='sandbox_matic'
        )
    ...
    def test_fetch_trading_pairs(self):
        trading_pairs: List[str] = self.run_async(
            self.matic_order_book_data_source.fetch_trading_pairs(domain='sandbox_matic'))
        self.assertIn("IDEX-USD", trading_pairs)
        self.assertIn("DIL-USD", trading_pairs)
        ...
        with patch('aiohttp.ClientSession.get', new_callable=AsyncMock) as mocked_get:
            for t_pair in self.matic_sample_pairs:
                ...
                last_traded_price: float = self.run_async(
                    self.matic_order_book_data_source.get_last_traded_price(t_pair, "https://api-sandbox-matic.idex.io"))
                self.assertEqual(0.01780000, last_traded_price)

    @patch('hummingbot.connector.exchange.idex.idex_resolve.get_idex_rest_url')
    @patch('aiohttp.ClientSession.get', new_callable=AsyncMock)
    def test_get_last_traded_prices(self, mocked_get, mocked_api_url):
        mocked_api_url.return_value = "https://api-sandbox-matic.idex.io"
        mocked_get.return_value.status = 200
        mocked_get.return_value.json.return_value = FixtureIdex.TRADING_PAIR_TRADES
        last_traded_prices: List[str] = self.run_async(
            self.matic_order_book_data_source.get_last_traded_prices(self.matic_sample_pairs))
        self.assertEqual({"IDEX-USD": 0.01780000,
                         "DIL-USD": 0.01780000}, last_traded_prices)
    ...
    def test_get_mid_price(self, mocked_get, mocked_api_url):
        ...
        for t_pair in self.matic_sample_pairs:
            t_pair_mid_price = self.matic_order_book_data_source.get_mid_price(t_pair, domain='sandbox_matic')
    ...
    async def get_snapshot(self, trading_pair):
        async with aiohttp.ClientSession() as client:
            try:
                snapshot = await self.matic_order_book_data_source.get_snapshot(
                    client, trading_pair, domain='sandbox_matic'
                )
                return snapshot
            except Exception:
                return None
    ...
    def test_get_snapshot(self, mocked_json, mocked_api_url):
        ...
        snapshot = self.ev_loop.run_until_complete(self.get_snapshot("IDEX-USD"))
    ...
    @patch('hummingbot.connector.exchange.idex.idex_resolve.get_idex_rest_url')
    @patch('hummingbot.connector.exchange.idex.idex_api_order_book_data_source.IdexAPIOrderBookDataSource.get_snapshot')
    def test_get_new_order_book(self, mock_get_snapshot, mocked_api_url):
        ...
        mocked_api_url.return_value = "https://api-sandbox-matic.idex.io"
        orderbook = self.ev_loop.run_until_complete(self.matic_order_book_data_source.get_new_order_book("IDEX-USD"))
    ...
    def test_get_tracking_pairs(self, mock_get_snapshot, mocked_api_url):
        ...
        tracking_pairs = self.ev_loop.run_until_complete(self.matic_order_book_data_source.get_tracking_pairs())

        # Validate the number of tracking pairs is equal to the number of trading pairs received
        self.assertEqual(len(self.matic_sample_pairs), len(tracking_pairs))

        # Make sure the entry key in tracking pairs matches with what's in the trading pairs
        for trading_pair, tracking_pair_obj in zip(self.matic_sample_pairs, list(tracking_pairs.keys())):
        ...
        for trading_pair, order_book_tracker_entry in zip(self.matic_sample_pairs, tracking_pairs.values()):
    ...
    def test_listen_for_order_book_snapshots(self, mock_get_snapshot, mock_api_url):
        ...
        mock_api_url.return_value = "https://api-sandbox-matic.idex.io"
        ...
        try:
            self.run_async(
                ...
                asyncio.wait_for(
                    self.matic_order_book_data_source.listen_for_order_book_snapshots(ev_loop=self.ev_loop, output=q),
                    timeout=timeout
                )
            )
        ...
        self.assertEqual(first_item.content['trading_pair'], self.matic_sample_pairs[0])
        self.assertEqual(first_item.content['sequence'], FixtureIdex.SNAPSHOT_1['sequence'])

    @patch(
        'hummingbot.connector.exchange.idex.idex_api_order_book_data_source.DISABLE_LISTEN_FOR_ORDERBOOK_DIFFS', False
    )
    @patch('hummingbot.connector.exchange.idex.idex_resolve.get_idex_ws_feed')
    @patch(
        'hummingbot.connector.exchange.idex.idex_api_order_book_data_source.IdexAPIOrderBookDataSource._inner_messages'
    )
    def test_listen_for_order_book_diffs(self, mock_inner_messages, mock_ws_feed):
        timeout = 3
        mock_ws_feed.return_value = "wss://websocket-sandbox-matic.idex.io/v1"
        ...
        try:
            self.run_async(
                # Force exit from event loop after set timeout seconds
                asyncio.wait_for(
                    self.matic_order_book_data_source.listen_for_order_book_diffs(ev_loop=self.ev_loop, output=q),
                    timeout=timeout
                )
            )
        except asyncio.exceptions.TimeoutError as e:
    ...
    def test_listen_for_trades(self, mock_inner_messages, mock_ws_feed):
        ...
        try:
            self.run_async(
                # Force exit from event loop after set timeout seconds
                asyncio.wait_for(
                    self.matic_order_book_data_source.listen_for_trades(ev_loop=self.ev_loop, output=q),
                    timeout=timeout
                )
            )
        except asyncio.exceptions.TimeoutError as e:
